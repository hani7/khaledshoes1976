...
def setup_view(request):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='shoes_product_categories'")
            exists = cursor.fetchone()
            if not exists:
                cursor.execute("""
                    CREATE TABLE "shoes_product_categories" (
                ...
                """)
                log.append("✅ Table 'shoes_product_categories' CREATED via raw SQL")
            else:
                log.append("ℹ️  Table 'shoes_product_categories' already exists")
...
# Serve media files unconditionally (bypasses DEBUG=False restriction on cPanel)
urlpatterns += [
