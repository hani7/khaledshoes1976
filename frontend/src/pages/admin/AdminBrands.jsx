...
export default function AdminBrands() {
  ...
  const load = () => {
    setLoading(true)
    adminClient.get('/admin/brands/?page_size=100')
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (modal === 'add') await adminClient.post('/admin/brands/', fd, cfg)
      else await adminClient.patch(`/admin/brands/${editId}/`, fd, cfg)
      setModal(null); load()
    } catch (err) {
  ...
  return (
    <div>
      <h2 style={{ fontSize: '1.3rem', fontWeight: 700, marginBottom: 24 }}>Marques</h2>

      <div className="admin-card">
        <div className="admin-card-header">
          <span className="admin-card-title">{cats.length} marque{cats.length !== 1 ? 's' : ''}</span>
          <button className="btn-primary" onClick={openAdd} id="add-brand-btn">
          ...
          <div className="admin-table-wrap">
            <table className="admin-table">
              ...
              <tbody>
                ...
                {cats.length === 0 && (
                  <tr><td colSpan={7}><div className="admin-empty"><p>Aucune marque.</p></div></td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="admin-modal-overlay" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="admin-modal">
            <div className="admin-modal-header">
              <span className="admin-modal-title">{modal === 'add' ? 'Ajouter une marque' : 'Modifier la marque'}</span>
            ...
            <form onSubmit={handleSave}>
              <div className="admin-modal-body">
                ...
                <div className="form-group">
                  <label>Nom *</label>
                  <input className="form-control" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Nom de la marque" />
                </div>
                <div className="form-group">
