...
const printBarcode = (product) => {
  ...
  const priceStr = parseInt(product.effective_price || product.price).toLocaleString('fr-FR') + ' DA'
  ...
  printWindow.document.write(`
    ...
    <html>
      <head>
        <title>Impression Code Barre - ${nameStr}</title>
        <style>
          @media print {
            @page { size: 4cm 2cm; margin: 0; }
            body { margin: 0; padding: 0; width: 4cm; height: 2cm; overflow: hidden; display: flex; align-items: center; justify-content: center; font-family: sans-serif; background: white; -webkit-print-color-adjust: exact; }
          }
          body { display: flex; align-items: center; justify-content: center; font-family: sans-serif; background: white; width: 4cm; height: 2cm; margin: 0; padding: 0; overflow: hidden; }
          .label { width: 100%; height: 100%; display: flex; flex-direction: row; box-sizing: border-box; padding: 2mm; }
          .left { width: 40%; display: flex; flex-direction: column; justify-content: center; align-items: center; }
      ...
      <body>
        <div class="label">
          <div class="left">
            <div class="logo-text">KHALED<br><span class="logo-shoes">SHOES</span></div>
          </div>
          <div class="right">
...
export default function AdminProducts() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      let newProductId = editId
      if (modal === 'add') {
        const res = await adminClient.post('/admin/products/', fd, cfg)
        newProductId = res.data.id
      } else {
        await adminClient.patch(`/admin/products/${editId}/`, fd, cfg)
      }

      // Upload local variants if adding new product
      if (modal === 'add') {
        for (const v of variants) {
          if (v.id.toString().startsWith('temp_')) {
            const vFd = new FormData()
            vFd.append('product', newProductId)
            vFd.append('name', v.name)
            vFd.append('color_hex', v.color_hex)
            vFd.append('stock', v.stock)
            vFd.append('is_available', v.is_available)
            vFd.append('choice_group', v.choice_group || '')
            if (v.price) vFd.append('price', v.price)
            if (v.localFile) vFd.append('image', v.localFile)
            await adminClient.post('/admin/variants/', vFd, cfg)
          }
        }
        
        // Upload local gallery if adding new product
        for (const g of gallery) {
          if (g.id.toString().startsWith('temp_')) {
            const gFd = new FormData()
            gFd.append('product', newProductId)
            if (g.imageFile) gFd.append('image', g.imageFile)
            if (g.videoFile) gFd.append('video', g.videoFile)
            await adminClient.post('/admin/images/', gFd, cfg)
          }
        }
      }
  ...
  const handleSaveVariant = async (e) => {
    ...
    if (!editId) {
      // Local mode
      if (editVariantId) {
        setVariants(variants.map(v => v.id === editVariantId ? { ...v, name: newVariant.name, color_hex: finalColorHex, stock: newVariant.stock, price: newVariant.price, is_available: newVariant.is_available, choice_group: newVariant.choice_group, localFile: variantFile || v.localFile } : v))
      } else {
        setVariants([...variants, { id: 'temp_'+Date.now(), name: newVariant.name, color_hex: finalColorHex, stock: newVariant.stock, price: newVariant.price, is_available: newVariant.is_available, choice_group: newVariant.choice_group, localFile: variantFile }])
      }
      setShowVariantModal(false)
      cancelEditVariant()
      return
  ...
  const handleDeleteVariant = async (vid) => {
    ...
    if (!editId || vid.toString().startsWith('temp_')) {
  ...
  const handleSaveGallery = async () => {
    if (!galleryImageFile && !galleryVideoFile) return
    
    if (!editId) {
      setGallery([...gallery, { id: 'temp_'+Date.now(), imageFile: galleryImageFile, videoFile: galleryVideoFile }])
  ...
  const handleDeleteGallery = async (id) => {
    ...
    if (!editId || id.toString().startsWith('temp_')) {
                      ...
                      <tr key={p.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 6 }}>
                            ...
                            <button className="btn-action-icon" style={{ background: '#4b5563' }} onClick={() => printBarcode(p)} title="Imprimer Code Barre">
                ...
                {paginated.map(p => (
                  ...
                  >
                    ...
                    <div style={{ width: '100%', aspectRatio: '1', background: 'var(--admin-surface2)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
                      ...
                      {/* Promo badge */}
                      {p.is_promo && (
                        <span style={{ position: 'absolute', top: 8, left: 8, fontSize: '0.6rem', fontWeight: 700, padding: '2px 6px', borderRadius: 20, background: '#f2b01e', color: '#fff' }}>PROMO</span>
                      )}
                    </div>
                    ...
                    <div style={{ display: 'flex', borderTop: '1px solid var(--admin-border)' }}>
                      ...
                      <button
                        ...
                        className="admin-grid-action-btn"
                        style={{
                          flex: 1, padding: '10px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                          color: 'var(--admin-text-muted)', fontSize: '0.8rem', fontWeight: '500', cursor: 'pointer',
                          background: 'transparent', border: 'none', borderRight: '1px solid var(--admin-border)', transition: 'background 0.15s, color 0.15s'
                        }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(107,114,128,0.07)'; e.currentTarget.style.color = '#4b5563' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--admin-text-muted)' }}
                      >
                        <Printer size={14} /> Code
                      </button>
                      <button
                        onClick={() => openEdit(p)}
                        title="Modifier"
            ...
            <div className="admin-modal-body" style={{ padding: 0, maxHeight: 'none', overflowY: 'visible' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px', alignItems: 'start' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    {/* Tarification */}
                    <div className="admin-card" style={{ padding: '24px', boxShadow: '0 2px 10px rgba(0,0,0,0.02)' }}>
                      <h3 style={{ fontSize: '1.15rem', fontWeight: 700, marginBottom: '20px', borderBottom: '1px solid var(--admin-border)', paddingBottom: '10px' }}>Tarification</h3>
                    ...
                    <div className="admin-card" style={{ padding: '24px', boxShadow: '0 2px 10px rgba(0,0,0,0.02)' }}>
                      ...
                      {!editId ? (
                        <div style={{ padding: '20px', background: 'rgba(0,0,0,0.03)', borderRadius: '8px', textAlign: 'center', color: 'var(--admin-text-muted)' }}>
                          <p style={{ margin: 0, fontWeight: 500 }}>Veuillez d'abord sauvegarder le produit pour pouvoir y ajouter des variations, des images à la galerie et des produits similaires.</p>
                        </div>
                      ) : (
                        <>
                        {/* Variations */}
                            ...
                            {variants.map(v => (
                              <div key={v.id} style={{ background: 'var(--admin-surface)', padding: '15px', borderRadius: '6px', marginBottom: '10px', display: 'flex', gap: '15px', alignItems: 'center', border: '1px solid var(--admin-border)' }}>
                                <div style={{ width: '40px', height: '40px', borderRadius: '4px', overflow: 'hidden', flexShrink: 0, border: '1px solid var(--admin-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: v.color_hex?.includes('|img_on') ? '#f0f0f0' : ((v.color_hex || '').replace('|img_on','').replace('|img_off','') || '#fff') }}>
                                  {v.color_hex?.includes('|img_on') && (v.localFile || v.image) ? <img src={v.localFile ? URL.createObjectURL(v.localFile) : mediaUrl(v.image)} alt="var" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (v.color_hex?.includes('|img_on') && (v.color_hex || '').replace('|img_on','').replace('|img_off','').startsWith('http') ? <img src={(v.color_hex || '').replace('|img_on','').replace('|img_off','')} alt="var" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : null)}
                                </div>
                                <div style={{ flex: 1 }}>
                                  ...
                                  <div key={g.id} style={{ position: 'relative', width: '100px', height: '100px', borderRadius: '6px', overflow: 'hidden', border: '1px solid var(--admin-border)' }}>
                                    {g.video || g.videoFile ? (
                                      <div style={{ width: '100%', height: '100%', position: 'relative', background: '#000' }}>
                                        {(g.image || g.imageFile) && <img src={g.imageFile ? URL.createObjectURL(g.imageFile) : mediaUrl(g.image)} alt="poster" style={{ opacity: 0.6 }} />}
                                      ...
                                      <img src={g.imageFile ? URL.createObjectURL(g.imageFile) : mediaUrl(g.image)} alt="galerie" />
                                    )}
