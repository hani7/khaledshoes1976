      ...
      <div className="container">
        ...
        <div className="checkout-form-wrap">
          ...
          <form onSubmit={handleSubmit} className="checkout-form" id="checkout-form">
                      ...
                      return (
                        <div key={c.id} className={`delivery-card ${Number(form.delivery_company_id) === c.id ? 'active' : ''}`} style={{ border: '1px solid var(--color-gray-200)', borderRadius: 'var(--radius-md)', padding: 16 }}>
                          ...
                          {Number(form.delivery_company_id) === c.id && (
                            <div style={{ paddingLeft: 28, display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
                              <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', fontWeight: 500, cursor: 'pointer' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <input 
                                    type="radio" 
                                    name={`delivery_type_${c.id}`} 
                                    value="home" 
                              ...
                              <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', fontWeight: 500, cursor: 'pointer' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <input 
                                    ...
                                    name={`delivery_type_${c.id}`} 
                                    value="desk" 
                                    checked={form.delivery_type === 'desk'} 
                                    onChange={() => handleChange('delivery_type', 'desk')} 
                                  />
                                  <span>Bureau / Relais (Stopdesk)</span>
                                </div>
            ...
            <button type="submit" className="btn btn-accent checkout-submit-btn" disabled={loading} id="submit-order-btn" style={{ fontSize: 'clamp(0.75rem, 3vw, 1rem)', letterSpacing: '0.03em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          ...
          <div className="checkout-summary__totals">
            ...
            <div className="checkout-summary__row checkout-summary__row--total" style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
              ...
              {(form.payment_method === 'cib') && deliveryCost > 0 && (
                <div style={{ fontSize: '0.85rem', color: '#f2b01e', fontWeight: 'bold', width: '100%', textAlign: 'right' }}>
