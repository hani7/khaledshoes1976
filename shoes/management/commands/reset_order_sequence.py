...
class Command(BaseCommand):
    help = "Réinitialise l'AUTO_INCREMENT de la table shoes_order"
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("SELECT MAX(id) FROM shoes_order")
            row = cursor.fetchone()
        ...
        with connection.cursor() as cursor:
            ...
            if db_engine == 'mysql':
                cursor.execute(f"ALTER TABLE shoes_order AUTO_INCREMENT = {next_id};")
                ...
                cursor.execute(
                    "UPDATE sqlite_sequence SET seq = %s WHERE name = 'shoes_order'",
                    [next_id - 1]
                )
                self.stdout.write(self.style.SUCCESS(
