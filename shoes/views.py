...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for item_data in items_data:
            ...
            elif variant and variant.price and not (product.is_promo and product.promo_price):
        ...
        # Email notifications nouvelle commande (asynchrone)
        ...
        def send_order_emails():
            try:
                ...
                import logging
                logging.getLogger('shoes').error(
                    f"Erreur email nouvelle commande #{order.id}: {_email_err}", exc_info=True
                )

        import threading
        threading.Thread(target=send_order_emails, daemon=True).start()
...
class AdminOrderViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if old_status != updated_instance.status:
            ...
            # --- MYLERZ AUTO-SHIP ---
            if updated_instance.status == 'confirmed' and not updated_instance.yalidine_tracking:
                from . import yalidine_service
                def async_yalidine_ship(order_id):
                    try:
                        from .models import Order, OrderStatusHistory
                        o = Order.objects.get(id=order_id)
                        res = yalidine_service.create_shipment(o)
                        if res.get('success'):
                            o.yalidine_tracking = res.get('barcode') or ''
                            o.yalidine_tracking = res.get('pickup_code') or ''
                            o.yalidine_status = 'Shipment Created'
                            o.save(update_fields=['yalidine_tracking', 'yalidine_tracking', 'yalidine_status'])
                            OrderStatusHistory.objects.create(
                                order=o,
                                status=o.status,
                                notes=f"Colis Yalidine g├®n├®r├® auto. Barcode: {o.yalidine_tracking}"
                            )
                        else:
                            ...
                            OrderStatusHistory.objects.create(
                                order=o,
                                status=o.status,
                                notes=f"├ëchec de cr├®ation du colis Yalidine : {msg}"
                            )
                    except Exception as e:
                        pass
                import threading
                threading.Thread(target=async_yalidine_ship, args=(updated_instance.id,)).start()
    ...
    @action(detail=True, methods=['get'])
    def yalidine_ship_debug(self, request, pk=None):
        """Debug: build the Yalidine payload for a real order WITHOUT sending it."""
        order = self.get_object()
        from . import yalidine_service
        import datetime
        try:
            ...
            wilaya_normalized = yalidine_service.normalize_wilaya(raw_wilaya)
            # Safety: ensure wilaya_normalized is a known Yalidine city (mirrors create_shipment logic)
            if wilaya_normalized not in yalidine_service.WILAYA_LIST:
                import unicodedata
                key_s = ''.join(c for c in unicodedata.normalize('NFD', wilaya_normalized.lower()) if unicodedata.category(c) != 'Mn')
                for w in yalidine_service.WILAYA_LIST:
            ...
            city = wilaya_normalized                  # toujours la wilaya — Yalidine la reconnaît
            ...
            warehouse = yalidine_service._cfg_warehouse()
            payload = {
            ...
            return Response({
                'order_id': order.id,
                'yalidine_tracking_already_set': bool(order.yalidine_tracking),
                'yalidine_tracking': order.yalidine_tracking,
                'payload': payload,
                'warehouse': warehouse,
            })
        except Exception as e:
    ...
    def yalidine_test(self, request):
        """Diagnostic: test Yalidine auth + test AddOrders with minimal payload."""
        import requests as req_lib
        import datetime
        from . import yalidine_service
        result = {
            'username': yalidine_service.YALIDINE_API_ID or '(vide)',
            'password_set': bool(yalidine_service.YALIDINE_API_TOKEN),
            'warehouse': yalidine_service.MYLERZ_WAREHOUSE,
            'base_url': yalidine_service.MYLERZ_BASE_URL,
        }
        # Step 1: Auth
        token = None
        try:
            token = yalidine_service.get_yalidine_token()
            result['auth'] = 'OK'
        except Exception as e:
        ...
        test_payload = [{
            ...
            "Special_Notes": "",
            "Reference": "DIAG-001",
            "WarehouseName": yalidine_service.MYLERZ_WAREHOUSE,
            "AllowToOpenPackage": True,
            "ValueOfGoods": 500.0,
            "Country": "DZ",
        }]
        try:
            resp = req_lib.post(
                f"{yalidine_service.MYLERZ_BASE_URL}/api/Orders/AddOrders",
                json=test_payload,
                headers=headers,
                timeout=20,
            )
    ...
    def yalidine_ship(self, request, pk=None):
        order = self.get_object()
        if order.yalidine_tracking:
            return Response({'error': 'Ce colis a d├®j├á un code-barres Yalidine.'}, status=400)
        from . import yalidine_service
        try:
            res = yalidine_service.create_shipment(order)
        except Exception as e:
            import traceback
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"yalidine_ship exception for order #{order.id}: {traceback.format_exc()}")
            return Response({'success': False, 'message': f'Erreur serveur Yalidine: {e}'}, status=400)
        if res.get('success'):
            order.yalidine_tracking = res.get('barcode') or ''
            order.yalidine_tracking = res.get('pickup_code') or ''
            order.yalidine_status = 'Shipment Created'
            order.save(update_fields=['yalidine_tracking', 'yalidine_tracking', 'yalidine_status'])
            OrderStatusHistory.objects.create(
                order=order,
                status=order.status,
                notes=f"Colis Yalidine g├®n├®r├® manuellement. Barcode: {order.yalidine_tracking}"
            )
            return Response(res)
        return Response(res, status=400)

    @action(detail=True, methods=['get'])
    def yalidine_track(self, request, pk=None):
        order = self.get_object()
        if not order.yalidine_tracking:
            return Response({'error': 'Aucun code-barres Yalidine pour cette commande.'}, status=400)
        from . import yalidine_service
        import logging
        _logger = logging.getLogger(__name__)

        # ── Mapping statut Yalidine Algeria → statut Piové ──────────────────
        ...
        res = yalidine_service.track_shipment(order.yalidine_tracking)
        _logger.info(f"yalidine_track raw response for #{order.id}: {res}")

        if res.get('success'):
            tracking = res.get('tracking', [])
            _logger.info(f"yalidine_track tracking list for #{order.id}: {tracking}")

            if tracking:
                # ── Extraire le statut depuis tous les champs possibles Yalidine ──
                first = tracking[0]
                _logger.info(f"yalidine_track first event keys: {list(first.keys()) if isinstance(first, dict) else first}")
                ...
                if not new_yalidine_status and isinstance(first, dict):
                ...
                _logger.info(f"yalidine_track extracted status for #{order.id}: '{new_yalidine_status}'")

                # Mettre à jour yalidine_status dans la DB
                if new_yalidine_status and order.yalidine_status != new_yalidine_status:
                    order.yalidine_status = new_yalidine_status
                    order.save(update_fields=['yalidine_status'])

                # Mapper vers statut Piové
                mapped = MYLERZ_TO_PIOVE.get(new_yalidine_status.lower().strip())
                if not mapped:
                    # Essai partiel (ex: "Ready In Forward Delivery" vs "Ready in Forward delivery")
                    sl = new_yalidine_status.lower()
                ...
                if mapped and order.status != mapped:
                    ...
                    OrderStatusHistory.objects.create(
                        order=order,
                        status=mapped,
                        notes=f"Statut mis à jour via Yalidine : {new_yalidine_status}"
                    )
                    _logger.info(f"Order #{order.id}: {old_status} → {mapped} (Yalidine: {new_yalidine_status})")
    ...
    @action(detail=True, methods=['get'])
    def yalidine_track_debug(self, request, pk=None):
        """Debug: retourne la réponse brute Yalidine TrackPackages pour ce colis."""
        order = self.get_object()
        if not order.yalidine_tracking:
            return Response({'error': 'Pas de barcode Yalidine.'}, status=400)

        import requests as req_lib
        from . import yalidine_service

        payload = [{"Barcode": order.yalidine_tracking}]
        try:
            token = yalidine_service.get_yalidine_token()
            headers = {
                'Authorization': f'Bearer {token}',
            ...
            resp = req_lib.post(
                f"{yalidine_service.MYLERZ_BASE_URL}/api/packages/TrackPackages",
                json=payload,
                headers=headers,
                timeout=20,
            )
            return Response({
                'barcode': order.yalidine_tracking,
    ...
    @action(detail=True, methods=['post'])
    def yalidine_cancel(self, request, pk=None):
        order = self.get_object()
        if not order.yalidine_tracking:
            return Response({'error': 'Aucun code-barres Yalidine pour cette commande.'}, status=400)
        from . import yalidine_service
        res = yalidine_service.cancel_shipment(order.yalidine_tracking)
        if res.get('success'):
            order.yalidine_status = 'Cancelled on Yalidine'
            order.save(update_fields=['yalidine_status'])
            OrderStatusHistory.objects.create(
                order=order,
                status=order.status,
                notes=f"Envoi Yalidine annul├®."
            )
            return Response(res)
        return Response(res, status=400)

    @action(detail=False, methods=['post'])
    def bulk_yalidine_ship(self, request):
        try:
            ...
            if not yalidine_service.YALIDINE_API_ID or not yalidine_service.YALIDINE_API_TOKEN:
                return Response({'error': 'Credentials Yalidine non configur├®s sur le serveur (MYLERZ_USERNAME / MYLERZ_PASSWORD manquants dans le .env).'}, status=400)
            ...
            for order in orders:
                ...
                if res.get('success'):
                    ...
                    OrderStatusHistory.objects.create(
                        ...
                        notes=f"Colis Yalidine g├®n├®r├® en masse. Barcode: {order.yalidine_tracking}"
                    )
                res['id'] = order.id
                results.append(res)
    ...
    @action(detail=False, methods=['post'])
    def bulk_yalidine_track(self, request):
        ids = request.data.get('ids', [])
        if not ids:
            return Response({'error': 'Aucun ID fourni.'}, status=400)
        orders = Order.objects.filter(id__in=ids).exclude(yalidine_tracking='')
        from . import yalidine_service
        updated = 0
        
        import time
        for order in orders:
            time.sleep(0.25)  # Rate limit: 5 requests per second max
            res = yalidine_service.track_shipment(order.yalidine_tracking)
            if res.get('success'):
                tracking = res.get('tracking', [])
                if tracking:
                    ...
                    if new_status and order.yalidine_status != new_status:
                        order.yalidine_status = new_status
                        order.save(update_fields=['yalidine_status'])
                        updated += 1
        return Response({'success': True, 'updated': updated})

    @action(detail=False, methods=['post'])
    def bulk_yalidine_cancel(self, request):
        ids = request.data.get('ids', [])
        if not ids:
            return Response({'error': 'Aucun ID fourni.'}, status=400)
        orders = Order.objects.filter(id__in=ids).exclude(yalidine_tracking='')
        ...
        for order in orders:
            ...
            if res.get('success'):
                order.yalidine_status = 'Cancelled on Yalidine'
                order.save(update_fields=['yalidine_status'])
                OrderStatusHistory.objects.create(order=order, status=order.status, notes="Envoi Yalidine annul├® (en masse).")
            res['id'] = order.id
            results.append(res)
        return Response({'results': results})
    ...
    def unviewed_counts(self, request):
        ...
        normal_count = unviewed_orders.count()
        return Response({
            'normal': normal_count,
            'b2b': 0
        })
    ...
    def bulk_update_status(self, request):
        ...
        for order in orders:
            if order.status != new_status:
                ...
                # --- MYLERZ AUTO-SHIP ---
                if new_status == 'confirmed' and not order.yalidine_tracking:
                    from . import yalidine_service
                    def async_yalidine_ship(order_id):
                        try:
                            from .models import Order, OrderStatusHistory
                            ...
                            if res.get('success'):
                                ...
                                OrderStatusHistory.objects.create(
                                    order=o,
                                    status=o.status,
                                    notes=f"Colis Yalidine g├®n├®r├® auto (en masse). Barcode: {o.yalidine_tracking}"
                                )
                            else:
                                msg = res.get('message', 'Erreur inconnue')
                                OrderStatusHistory.objects.create(
                            ...
                            pass
                    import threading
                    threading.Thread(target=async_yalidine_ship, args=(order.id,)).start()
    ...
    def bulk_packing_slips(self, request):
        ...
        # Yalidine AWB style HTML rendering for printing
        html = """
        <html>
        <head>
            <meta charset="utf-8">
        ...
        for o in orders:
            ...
            barcode_val = o.yalidine_tracking or f"CMD-{o.id}"
            ...
            yalidine_label = "Yalidine Tracking" if o.yalidine_tracking else "Order Barcode"
            ...
            <div class="label-container">
                ...
                <div class="barcode-box">
                    ...
                    <div style="font-size:10px; margin-top:2px;">{yalidine_label}</div>
                </div>
...
@api_view(['POST'])
@permission_classes([AllowAny])
def yalidine_webhook(request):
    """
    Webhook endpoint for Yalidine Algeria to push status updates.
    ...
    from shoes.management.commands.sync_yalidine import send_status_email
    ...
    for event in events:
        ...
        try:
            order = Order.objects.filter(yalidine_tracking=barcode).first()
            if not order and str(barcode).isdigit():
                order = Order.objects.filter(id=barcode).first()
                
            if order and order.yalidine_status != status_text:
                order.yalidine_status = status_text
                order.save(update_fields=['yalidine_status'])
                ...
                if new_piove_status and order.status != new_piove_status:
                    ...
                    OrderStatusHistory.objects.create(
                        ...
                        notes=f"Statut Yalidine mis ├á jour via Webhook : {status_text}"
                    )
                    
                    send_status_email(order, new_piove_status, order.yalidine_tracking)
                    updated += 1
        except Exception as e:
            pass
...
class AdminMediaView(APIView):
    ...
    def get(self, request):
        ...
        api_base = getattr(settings, 'API_URL', request.build_absolute_uri('/')[:-1]).rstrip('/')
        results = []
...
class AdminOrderHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    filterset_fields = ['status', 'payment_status', 'is_deleted', 'payment_method']
    search_fields = ['guest_name', 'guest_phone', 'user__username', 'user__first_name', 'id']
...
class TrackOrderView(APIView):
    ...
    Returns order status + Yalidine live tracking events.
    ...
    def get(self, request):
        ...
        results = []
        for order in qs:
            # Fetch live Yalidine tracking if barcode exists
            tracking_events = []
            yalidine_current_status = order.yalidine_status or ''

            if order.yalidine_tracking:
                try:
                    from .yalidine_service import track_shipment
                    track_result = track_shipment(order.yalidine_tracking)
                    if track_result.get('success'):
                        ...
                        # Update yalidine_status in DB with latest event
                        if tracking_events:
                            latest = tracking_events[0].get('status', '')
                            if latest and latest != order.yalidine_status:
                                order.yalidine_status = latest
                                order.save(update_fields=['yalidine_status'])
                            yalidine_current_status = latest
                except Exception as e:
                    logger.warning(f"TrackOrderView: Yalidine tracking failed for barcode {order.yalidine_tracking}: {e}")
            ...
            results.append({
                ...
                'items': items_data,
                'yalidine_tracking': order.yalidine_tracking or None,
                'yalidine_status': yalidine_current_status or None,
