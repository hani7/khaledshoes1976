...
# Max ID actuel
cur.execute("SELECT MAX(id) FROM shoes_order")
...
if cur.fetchone():
    cur.execute("SELECT seq FROM sqlite_sequence WHERE name='shoes_order'")
    row = cur.fetchone()
    if row:
        cur.execute("UPDATE sqlite_sequence SET seq = ? WHERE name = 'shoes_order'", (next_id - 1,))
        print(f"sqlite_sequence mis a jour : seq = {next_id - 1}")
    else:
        cur.execute("INSERT INTO sqlite_sequence (name, seq) VALUES ('shoes_order', ?)", (next_id - 1,))
